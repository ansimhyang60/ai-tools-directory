...
import { archiveFeed } from "@/lib/archiveFeed";
import { paginateArchiveItems } from "@/lib/archiveSeed";
...
export default function ArchivePage() {
  ...
  const fallbackItems = archiveFeed.filter((item) => { const matchesType = !type || item.type === type; const haystack = `${item.title} ${item.summary} ${item.organization}`.toLowerCase(); return matchesType && (!query.trim() || haystack.includes(query.trim().toLowerCase())); });
  const fallbackPage = paginateArchiveItems(fallbackItems, page, 24);
  const visibleItems = apiItems.length ? apiItems : fallbackPage.items;
  const showingFallback = !apiItems.length;
  return <main className="site-shell inner-page archive-page">
    <header className="topbar"><Link className="brand" href="/"><span className="brand-stamp"><span className="stamp-slash">/</span><span className="stamp-main">AI<span>100</span></span><span className="stamp-caption">FIELD<br />GUIDE</span></span></Link><nav className="topnav"><Link href="/workflows">업무 찾기</Link><Link href="/tools">도구 찾기</Link><Link href="/skills">스킬 라이브러리</Link><Link className="active" href="/archive">기회·뉴스</Link></nav><MobileNav active="/archive" /><div className="top-actions"><GlobalSearchTrigger /><Link className="back-home" href="/"><ArrowUpRight size={15} /> 홈</Link></div></header>
    <section className="inner-hero archive-hero"><div><span className="section-kicker">AI OPPORTUNITY / LIVE ARCHIVE</span><h1>공모전부터<br /><em>활용사례까지.</em></h1><p>공식 기관의 AI 공모전·정부지원·전시·뉴스·활용사례를 원문 링크와 함께 모읍니다. 새 항목은 자동 갱신됩니다.</p></div><div className="archive-hero-stamp"><strong>{data?.total ?? archiveFeed.length}</strong><span>indexed<br />records</span></div></section>
