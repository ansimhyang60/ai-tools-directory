...
export default function WorkflowsPage() {
  ...
  const filtered = useMemo(() => { const term = query.trim().toLowerCase(); const base = allWorkflowPrompts.filter((item) => { const matchesScope = scope === "전체" || (scope === "추천" && featuredIds.has(item.id)) || (scope === "공개 카탈로그" && sourceById[item.id]); const matchesFavorite = !favoriteOnly || favorites.includes(item.id); const matchesLevel = level === "전체" || levelById[item.id] === level; const matchesCategory = category === "전체" || item.category === category; const matchesQuery = !term || [item.title, item.category, item.tools.join(" "), item.purpose].join(" ").toLowerCase().includes(term); return matchesScope && matchesLevel && matchesCategory && matchesQuery && matchesFavorite; }); return limit === "전체" ? base : base.slice(0, Number(limit));   }, [category, favoriteOnly, favorites, level, limit, query, scope]);
