...
const categoryTaxonomy = [
  ["업무용", "고객·영업", "CRM"], ["업무용", "전사 운영", "ERP"], ["업무용", "물류·재고", "재고관리"], ["업무용", "업무 자동화", "업무 자동화"], ["업무용", "고급 자동화", "고급 업무 자동화"], ["업무용", "경리·회계", "경리·회계"], ["업무용", "협업·보고", "협업·보고"], ["업무용", "고객 응대", "고객 응대"], ["업무용", "기획·검수", "기획·검수"],
  ["홈페이지·앱", "웹앱 제작", "웹앱 제작"], ["홈페이지·앱", "화면 설계", "UI·UX 디자인"], ["홈페이지·앱", "배포·검색", "배포·운영"], ["홈페이지·앱", "배포·검색", "SEO·웹"],
  ["데이터·엑셀", "데이터 분석", "데이터 분석"], ["데이터·엑셀", "시트 업무", "시트·문서"], ["데이터·엑셀", "엑셀 자동화", "엑셀 자동화"],
  ["마케팅·콘텐츠", "콘텐츠 제작", "콘텐츠·바이럴"], ["마케팅·콘텐츠", "마케팅·상품", "마케팅·상품"], ["마케팅·콘텐츠", "마케팅·상품", "마케팅"],
  ["리서치·학습", "조사·리서치", "리서치"], ["리서치·학습", "학습", "학습·리서치"], ["리서치·학습", "프롬프트 설계", "프롬프트 설계"], ["리서치·학습", "발표·교육", "발표·교육"], ["리서치·학습", "번역", "번역"],
...
export default function WorkflowsPage() {
  ...
  const filtered = useMemo(() => { const term = query.trim().toLowerCase(); const base = allWorkflowPrompts.filter((item) => { const matchesScope = scope === "전체" || (scope === "추천" && featuredIds.has(item.id)) || (scope === "공개 카탈로그" && sourceById[item.id]); const matchesFavorite = !favoriteOnly || favorites.includes(item.id); const matchesLevel = level === "전체" || getLevel(item) === level; const matchesCategory = category === "전체" || item.category === category; const hierarchy = categoryTaxonomy.find((entry) => entry.specific === item.category); const matchesMajor = majorCategory === "전체" || hierarchy?.major === majorCategory; const matchesMiddle = middleCategory === "전체" || hierarchy?.middle === middleCategory; const matchesQuery = !term || [item.title, item.category, item.tools.join(" "), item.purpose].join(" ").toLowerCase().includes(term); return matchesScope && matchesLevel && matchesCategory && matchesMajor && matchesMiddle && matchesQuery && matchesFavorite; }); return limit === "전체" ? base : base.slice(0, Number(limit));   }, [category, favoriteOnly, favorites, level, limit, majorCategory, middleCategory, query, scope]);
