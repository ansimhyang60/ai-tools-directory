...
function ResultCard({ item }: { item: SearchRecord }) {
  return <Link className="global-search-result" href={item.href}>
    ...
    <h2>{item.title}</h2>
    <p>{item.summary}</p>
    <div className="global-search-result-foot"><span>{item.source ?? BRAND_NAME}</span><span className="global-search-trust">{getSearchTrustLabel(item.trustScore)} · {formatDate(item.updatedAt)}</span><ArrowUpRight size={16} /></div>
  </Link>;
}
...
export function GlobalSearchPage() {
  ...
  return <PageFrame active="" kicker="GLOBAL INDEX / SEARCH EVERYTHING" title={<>찾고 싶은 것을<br /><em>한 번에 찾기.</em></>}>
    <section className="global-search-hero" aria-labelledby="global-search-title">
      <div className="global-search-hero-copy"><span className="section-kicker">AI 활용 가이드 / 전체 검색</span><h2 id="global-search-title">도구만 찾지 말고,<br /><em>일의 맥락까지.</em></h2><p>AI 도구, SKILL, UI Guide, 디자인 시스템, 업무 워크플로우와 프롬프트, 공식 아카이브를 한 번에 검색합니다.</p></div>
