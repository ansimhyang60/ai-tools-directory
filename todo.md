...
- [ ] 공유 Vercel 화면과 기존 업무·도구·스킬 데이터 구조 확인
- [ ] 업무별 관계 모델과 연결 규칙 설계
- [ ] 클릭 강조 가능한 mind map/tree 컴포넌트 구현
- [ ] 홈 최상단 full-bleed 배치 및 모바일 트리 전환
- [ ] 키보드·접근성·반응형·빌드 검증
- [ ] GitHub 자동 배포 및 체크포인트 저장

## 첨부 예시형 워크트리·마인드맵 재설계

- [ ] 첨부 예시의 중심 노드·장·가지·연결선 구조 기록
- [ ] 업무별 툴·SKILL·프롬프트·워크플로우 관계를 방사형 데이터로 재구성
- [ ] 손그림식 노드·연결선·메모 라벨 UI 구현
- [ ] 노드 클릭 시 해당 가지와 연결 항목 강조
- [ ] 데스크톱 full-screen 및 모바일 세로 트리 검증
- [ ] GitHub push와 체크포인트 저장
