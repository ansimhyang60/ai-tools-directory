...
- [ ] 업무 진단 마법사 입력·추천 규칙·결과 화면 설계
- [ ] 도구 비교함 선택·삭제·최대 4개 비교 UI 구현
- [ ] 무료 플랜·가격 상태 필터와 도구 데이터 메타데이터 연결
- [ ] 출처·업데이트 날짜·검증 상태 신뢰도 표시 강화
- [ ] 실제 워크플로 템플릿 탐색·상세·복사 기능 구현
- [ ] 프롬프트 플레이그라운드 입력·조립·복사 기능 구현
- [ ] 접근성·반응형·상호작용·타입·Vitest·프로덕션 빌드 검증
- [ ] GitHub main 반영 및 Vercel 자동 공개 체크포인트 저장
